...
class Fap(nn.Module):
    def __init__(self, in_channel, mode) -> None:
        ...
        if mode[0] == 'train':
            self.gap = nn.AdaptiveAvgPool2d((1, 1))  # 训练模式下使用自适应平均池化
        elif mode[0] == 'test':
            # 测试模式下根据 mode[1] 参数选择不同的平均池化操作
            if mode[1] == 'HIDE':
                self.gap = AvgPool2d(base_size=110)  # 使用特定大小的平均池化
            elif mode[1] == 'GOPRO':
                self.gap = AvgPool2d(base_size=80)  # 使用特定大小的平均池化
            elif mode[1] == 'RSBlur':
                self.gap = AvgPool2d(base_size=75)  # 使用特定大小的平均池化
    def forward(self, x):
        # 执行全局平均池化操作
        x_d = self.gap(x)
        # 计算水平方向的高频部分
    ...
    def __init__(self, mode, inchannel, patch_size):
        ...
        if mode[0] == 'train':
            ...
            if mode[1] == 'HIDE':
                ...
                self.ap = AvgPool2d(base_size=75)
        # 设置 patch 大小和通道数
